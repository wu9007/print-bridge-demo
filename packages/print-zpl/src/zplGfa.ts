...
const FIELD = /\^(FO|FT)(-?\d+),(-?\d+)((?:\s*\^(?!FD|FS|FO|FT|XA|XZ)[^^]+)*)\s*\^FD([^^]*)\^FS/g;
...
async function textToGfa(
  ...
  const rawBox = keepBox
    ? null
    : inkBounds(ctx.getImageData(0, 0, width, height).data, width, height);
  ...
  const { bytes, bytesPerRow } = packMonoBitmap(
    oriented
      ...
      .getImageData(0, 0, oriented.width, oriented.height).data,
    oriented.width,
    oriented.height
  );
