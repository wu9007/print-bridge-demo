...
type PrintZplClient = {
  printZpl(req: { printerName: string; zpl?: string; zplBase64?: string }): Promise<void>;
};
...
function assertTemplateText(templateText: string): string {
  ...
  const looksLikePath =
    !/\^XA/i.test(text) &&
    (/\.zpl$/i.test(text) || /^(\.\/|\.\.\/|\/|[A-Za-z]:[\\/])/.test(text));
  if (looksLikePath) {
    throw new Error("请传入模板内容，不要传文件路径。业务仓用 ?raw 引入后再交给 printZpl。");
  }
  return text;
}

/** 研发设什么就替换什么，然后转中文、交给托盘。第三参是模板 text，不是路径。 */
